...
namespace COMPONENT {
void ConcentratedMoment::SetComp(const file_data &datas) {
...
void ConcentratedMoment::GenLoadVec(const MODEL::Model &model) {
  // 通过节点个数判断载荷列阵的维度
  int num_node = model._node.size();
  _load_vec = Eigen::VectorXd::Zero(6*num_node);
  // 设置载荷向量
  _load_vec[6 * num_node - 3] = _scale * _x;
  _load_vec[6 * num_node - 2] = _scale * _y;
  _load_vec[6 * num_node - 1] = _scale * _z;
...
Eigen::VectorXd ConcentratedMoment::GetGLobalLoad(const MODEL::Model &model) {
  // 载荷坐标系
